...
1. You upload one or more `.xlsx` / `.ods` manifesto files or `.csv` Base de Dados exports in the web UI  
2. Each Excel/ODS sheet (except templates like `Base de Dados` / `xxxx…`) or CSV flight group becomes a **flight**  
...
Upload: `POST /api/upload` (multipart field `file` — `.xlsx` / `.xlsm` / `.xls` / `.ods` / `.csv`).
